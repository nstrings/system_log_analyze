...
def export_csv(base_path, ip_counts, msg_counts, sev_counts, proc_counts):
...
def export_md(path, ip_counts, msg_counts, sev_counts, proc_counts, top_ips, top_msgs, top_procs):
...
def streamlit_ui():
    ...
    # Severity bar chart
    sev_df = pd.DataFrame(
        [{"severity":k.title(), "count":v} for k,v in sev_counts.items() if v>0]
    ...
    if top_msgs and msg_counts:
        ...
        msg_df = pd.DataFrame(msg_counts.most_common(top_msgs), columns=["message","count"])
...
def analyze_log(file_path, top_ips=5, top_msgs=5, grep=None, exclude=None, since=None, until=None, top_procs=5, export_csv_base=None, export_md_path=None):
    ...
    for line in lines:
        if xpat and xpat.search(line): 
            continue
        if pat and not pat.search(line):
            pass  # keep only if grep given? 
        if pat and not pat.search(line):
            # skip lines that don't match grep
            continue
        if since_dt or until_dt:
            ts = parse_line_ts(line)
            if ts:
    ...
    if export_csv_base:
        paths = export_csv(export_csv_base, ip_counts, msg_counts, sev_counts, proc_counts)
        print(f"\nCSV exported: {', '.join(paths)}")
    if export_md_path:
        export_md(export_md_path, ip_counts, msg_counts, sev_counts, proc_counts, top_ips, top_msgs, top_procs)
...
def cli():
    ...
    analyze_log(args.file, top_ips=args.top, top_msgs=args.msgs, grep=args.grep,
                exclude=args.exclude, since=args.since, until=args.until, top_procs=args.top_procs,
                export_csv_base=args.export_csv, export_md_path=args.export_md)
